...
export const roomRoutes: Record<string, RouteHandler> = {
  "/api/rooms": async (req: Request) => {
    ...
    if (req.method === "GET") {
      ...
      ({ data: roomRows, error } = await supabase
        .from("rooms")
        .select(
          "id, name, is_private, is_active, max_participants, room_type, updated_at, avatar_url",
        )
        .eq("is_active", true)
      ...
      if (error?.code === MISSING_COLUMN_ERROR_CODE) {
        ({ data: roomRows, error } = await supabase
          .from("rooms")
          .select(
            "id, name, is_active, max_participants, room_type, updated_at",
          )
          .eq("is_active", true)
          .order("updated_at", { ascending: false })
          .limit(100));
      ...
      if (error) {
        if (error.code === MISSING_TABLE_ERROR_CODE) {
          const fallbackRooms = Array.from(rooms.entries()).map(
            ([roomId, participants]) => ({
              id: roomId,
              name: roomId,
              isPrivate: false,
      ...
      try {
        ...
        if (hasAvatarFile) {
          ...
          try {
            ...
            const uploadResult = await uploadRoomAvatar(
              roomId,
              processed.buffer,
            ...
            const previousKey = extractAvatarKey(
              normalizeAvatarUrl(room.avatar_url),
            );
    ...
    if (req.method === "DELETE") {
      try {
        ...
        if (participantsError) {
          console.warn(
            "[Rooms] Failed to close participants:",
        ...
        if (
          ...
          participantsDeleteError.code !== MISSING_TABLE_ERROR_CODE
        ) {
  ...
  "/api/rooms/guest-link": async (req: Request) => {
    ...
    try {
      ...
      const guestLinkTtlSeconds = 300;
      const { token: guestToken, payload } = createGuestToken(roomId, {
        ttlSeconds: guestLinkTtlSeconds,
        allowPrivate: true,
      });
  ...
  "/api/rooms/invite-link/accept": async (req: Request) => {
    ...
    try {
      ...
      const addResult = await addRoomMember(
        room.id,
        userId,
        ROOM_MEMBER_ROLE_MEMBER,
        null,
  ...
  "/api/rooms/requests": async (req: Request) => {
    ...
    if (req.method === "POST") {
      ...
      if (!targetMembership.supported)
        return errorResponse("Room membership unsupported", 501);
      if (targetMembership.error)
        return roomStorageError(targetMembership.error);
      if (targetMembership.role)
        return errorResponse("User already a member", 409);
    ...
    if (req.method === "PATCH") {
      ...
      if (!ROOM_INVITE_ACTIONS.has(action))
      ...
      const { data: updated, error: updateError } = await supabase
        ...
        .eq("id", requestId)
        .select(
          "id, room_id, requester_id, target_id, status, created_at, responded_at",
        )
        .single();
  ...
  "/api/rooms/members": async (req: Request) => {
    ...
    if (req.method === "GET") {
      ...
      if (!membership.role && room.created_by !== userId)
        return errorResponse("Forbidden", 403);
    ...
    if (req.method === "PATCH") {
      ...
      if (!["promote", "demote", "remove", "leave"].includes(action))
        return errorResponse("Invalid action", 400);
      ...
      if (action === "leave") {
        ...
        if (
          participantsError &&
          participantsError.code !== MISSING_TABLE_ERROR_CODE
        ) {
          console.warn(
            "[Rooms] Failed to close participant:",
            participantsError.message,
          );
        }
        ...
        if (activeRoom?.has(targetId)) {
          ...
          broadcastToRoom(
            roomId,
            { type: "room-user-left", roomId, userId: targetId },
            targetId,
          );
        }
      ...
      const nextRole =
        action === "promote" ? ROOM_MEMBER_ROLE_ADMIN : ROOM_MEMBER_ROLE_MEMBER;
  ...
  "/api/rooms/messages": async (req: Request) => {
    ...
    if (req.method === "GET") {
      ...
      if (!access.ok)
        return errorResponse(
          ...
          access.error === "Room not found" ? 404 : 403,
        );
    ...
    if (req.method === "POST") {
      ...
      if (messageBody.length > ROOM_MESSAGE_MAX_LENGTH)
  ...
  "/api/rooms/mine": async (req: Request) => {
    ...
    if (req.method === "GET") {
      ...
      const roomSelectFallback =
        "id, name, is_active, max_participants, room_type, updated_at, created_by";
      ...
      const fetchRooms = async (applyFilters: (builder: any) => any) => {
        ...
        if (error?.code === MISSING_COLUMN_ERROR_CODE) {
          ({ data, error } = await applyFilters(
            supabase.from("rooms").select(roomSelectFallback),
          )
            .order("updated_at", { ascending: false })
            .limit(200));
        }
      ...
      const membershipRoomIds = Array.from(membershipMap.keys()).filter(
        Boolean,
      );

      if (membershipSupported && membershipRoomIds.length > 0) {
        const { data, error } = await fetchRooms((builder) =>
          builder.in("id", membershipRoomIds),
        );
        if (error) {
          if (error.code !== MISSING_TABLE_ERROR_CODE) {
            return errorResponse(error.message, 500);
      ...
      const normalizedRooms = orderedRooms.map((room) => {
        ...
        return {
          ...
          role:
            role ||
            (isCreator ? ROOM_MEMBER_ROLE_ADMIN : ROOM_MEMBER_ROLE_MEMBER),
          isCreator,
        };
      });
